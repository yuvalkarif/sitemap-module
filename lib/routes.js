...
function getStaticRoutes(router, options) {
  ...
  let flatStaticRoutes = flattenStaticRoutes(router)
  if (options?.routerCallback && typeof options.routerCallback === 'function') {
    flatStaticRoutes = options.routerCallback(flatStaticRoutes)
  }
  return flatStaticRoutes
